...
class AssociateIPAction(workflows.Action):
    class_id = forms.DynamicTypedChoiceField(label=_("IP Address"),
...
class AssociateIP(workflows.Step):
    ...
    contributes = ("class_id", "instance_id", "ip_address")

    def contribute(self, data, context):
        context = super(AssociateIP, self).contribute(data, context)
        class_id = data.get('class_id', None)
        if class_id:
            ip_choices = dict(self.action.fields['class_id'].choices)
            context["ip_address"] = ip_choices.get(class_id, None)
        return context


class ENCDataWorkflow(workflows.Workflow):
    slug = "ip_association"
